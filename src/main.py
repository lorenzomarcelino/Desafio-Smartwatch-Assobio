...
# AJUSTE CRÍTICO: O arquivo está dentro da pasta 'data'
...
async def lifespan(app: FastAPI):
    ...
    if not DATA_PATH.exists():
        # DEBUG: Se não achar, mostra o que tem na pasta para a gente entender o erro
...
class PatientInput(BaseModel):
    ...
    Sleep_Disorder: str | None = Field(default="None", alias="Sleep Disorder")
...
class PredictionInput(BaseModel):
    ...
    daily_steps: int
    blood_pressure: str 
    # Adicionamos os campos faltantes baseados no seu treino anterior
...
# 3. DELETE - Remover Paciente (NOVO!)
...
def get_insights():
    try:
        ...
        # Se o insight for verdadeiro, quanto maior o estresse, maior a BP.
        # 
        ...
        # (Para ficar menos ruidoso, poderíamos agrupar por faixas etárias, 
        # mas aqui retornaremos a idade pura para ver a linha de tendência)
        insight_age_trend = (
        ...
        # --- 4. Insight: Pressão Média por Distúrbio (ALTERADO) ---
        ...
        # Espera-se que "Sleep Apnea" e "Insomnia" tenham médias maiores que "None".
        ...
        insight_disorder_activity = (
            ...
            .mean()
            .round(1)
            .sort_values(ascending=False) # Ordena para provar quem é maior
            .to_dict()
        )
...
# --- ENDPOINT PREDIÇÃO (Mantido do anterior) ---
...
def predict_disorder(data: PredictionInput):
    ...
    try:
        ...
        # 4. Vetor Final (Conforme notebook)
        features = [[
            data.age,              
            data.sleep_duration,    
            data.heart_rate,        
